...
/** Store-wide totals across all processing + completed orders (from Shopify). */
...
/** Store-wide (all paid Shopify orders) date+total pairs for the total revenue line. */
...
export default function AdminOverviewPage() {
  ...
  // IQON Supplements app revenue for the selected range (RevenueCat) — null when the app
  ...
  async function handleSync() {
    ...
    setSyncMessage(null);
    try {
      const res = await fetch("/api/affiliates/admin/portal-commerce-sync", {
        method: "POST",
        credentials: "include",
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        ...
        setSyncMessage(
          `Synced ${data.ingested}/${data.fetched} orders from Shopify.`
        );
        load(preset, false);
      }
        ...
        actions={
          <>
            ...
            <button
              ...
              title="Verify affiliate promo codes exist as Shopify coupons"
            >
            ...
            <button
              ...
              disabled={syncing}
              className="inline-flex items-center gap-2 glass-surface rounded-full px-4 py-2 text-[10px] uppercase tracking-[0.18em] font-sans text-[#20282c] hover:bg-white/80 transition-colors disabled:opacity-60"
              title="Pull recent orders from Shopify and match them to affiliates"
            >
