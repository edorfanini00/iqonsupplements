...
function orderRef(o: { matchType: string; shopifyOrderId: number | null; orderId: string }): string {
  return o.matchType === "bonus" ? "—" : `#${o.shopifyOrderId ?? o.orderId}`;
}
...
function PendingBalanceRow({
  ...
  async function removeOrder(o: PendingOrderRow) {
    const what =
      o.matchType === "bonus"
        ? `${o.customerName}`
        : `order #${o.shopifyOrderId ?? o.orderId}`;
    if (
      !window.confirm(
              ...
              >
                <div className="min-w-0">
                  <p className="font-medium truncate">
                    {o.matchType === "bonus"
                      ? o.customerName
                      : `#${o.shopifyOrderId ?? o.orderId} · ${o.customerName}`}
                  </p>
