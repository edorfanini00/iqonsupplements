...
export function HumanStory({skincare=false,compact=false,movement=false,imageSrc,imageAlt}:{skincare?:boolean;compact?:boolean;movement?:boolean;imageSrc?:string;imageAlt?:string}) {
  return <section className={`human-story ${skincare?"human-story-skin":""} ${compact?"human-story-compact":""}`}>
    <div className="human-story-image"><picture>{skincare&&<source media="(max-width:760px)" srcSet="/images/editorial/skin-ritual.webp"/>}<img src={imageSrc||(skincare?"/images/editorial/skin-ritual-desktop.webp":movement?"/images/editorial/iqon-pilates-editorial.webp":"/images/editorial/everyday-movement.webp")} alt={imageAlt||(skincare?"A woman taking a moment for her skincare routine":movement?"A woman tying her shoes before Pilates":"Two adults talking during a coastal walk")} width={skincare?3840:1800} height={skincare?2160:1200} loading="lazy"/></picture></div>
    <div className="human-story-copy"><p className="eyebrow">{skincare?"A MOMENT THAT’S YOURS":movement?"MAKE TIME FOR YOU":"FOR THE LIFE YOU LIVE"}</p><h2>{skincare?<>A little care.<br/>Every day.</>:movement?<>Small rituals.<br/><span>A fuller life.</span></>:<>More room<br/>for living.</>}</h2><p>{skincare?"At the beginning of the day. At the end of the evening. A few familiar steps, and a moment to come back to yourself.":movement?"Making it to the class. Getting outside. Taking a moment before the day begins. A routine is more than the products on your shelf.":"The walk you look forward to. The people you make time for. The everyday moments that make a routine worth keeping."}</p><Link className="under-link" href={skincare?"/journal/a-simple-skincare-routine":"/approach"}>{skincare?"Find your skincare rhythm":"Our point of view"}<ArrowUpRight size={17}/></Link></div>
  </section>;
}
...
export function JournalPreview({skincare=false}:{skincare?:boolean}) {
 const skincareImages:Record<string,{image:string;imageAlt:string}>={
  "a-simple-skincare-routine":{image:"/images/editorial/skincare-ritual-v25.webp",imageAlt:"A woman applying cream while holding IQON Copper Peptide Restore Cream"},
...
export function SupplementCampaign() {
 return <section className="product-campaign product-campaign-supplements">
  <picture><source media="(max-width:760px)" srcSet="/images/editorial/supplement-campaign-mobile-v12.webp"/><img src="/images/editorial/supplement-campaign-desktop-v12.webp" alt="IQON GLP-1 Support and Creatine Monohydrate on a cool silver-blue surface with directional studio light" width={2172} height={724} loading="lazy"/></picture>
  <div className="product-campaign-copy"><p className="eyebrow">THE SUPPLEMENT COLLECTION</p><h2>A few essentials.<br/><span>A moment for you.</span></h2><Link className="button button-light" href="/collections/supplements">Explore supplements<ArrowUpRight size={16}/></Link></div>
 </section>;
}
