# Editable IQON precision detail v2 — Blender 5.2
# Run: blender --background --python create-precision-scene-v2.py
...
from mathutils import Vector
scene=bpy.context.scene
drop=bpy.data.objects['One serum droplet | 27 mm contact diameter']
# Preserve the editable drop and machined-disc composition; improve clarity and tonal control.
scene.camera.data.dof.use_dof=False
scene.camera.data.lens=74
scene.camera.rotation_euler=(Vector((.009,.004,.006))-scene.camera.location).to_track_quat('-Z','Y').to_euler()
power={'Key | long cool softbox':5.0,'Rim | precision vertical strip':4.8,'Fill | controlled frontal bounce':0.42,'Glint | narrow rear silver edge':1.4}
for name,value in power.items(): bpy.data.objects[name].data.energy=value
steel=bpy.data.materials['Silver | finely machined satin steel']
p=steel.node_tree.nodes.get('Principled BSDF')
p.inputs['Base Color'].default_value=(.32,.36,.41,1)
p.inputs['Roughness'].default_value=.32
for n in steel.node_tree.nodes:
    if n.type=='BUMP':
        n.inputs['Strength'].default_value=.22
        n.inputs['Distance'].default_value=.000026
    if n.type=='TEX_WAVE':
        n.inputs['Scale'].default_value=110
gp=bpy.data.materials['Serum | clear physical dielectric'].node_tree.nodes.get('Principled BSDF')
gp.inputs['Roughness'].default_value=.025
...
bpy.ops.wm.save_as_mainfile(filepath=str(output_dir/'iqon-precision-detail-v2.blend'))
target=artifacts.file(name='iqon-precision-detail-v2.png',media_type='image/png')
scene.render.filepath=target.path
bpy.ops.render.render(write_still=True)
target.publish()
result={'changes':'Disabled shallow depth of field, controlled highlights, reduced fill, darker silver with stronger measured machining texture, 48 sample adaptive Cycles CPU render','resolution':[1280,960],'samples':48}
