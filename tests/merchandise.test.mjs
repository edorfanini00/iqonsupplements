...
const {mergeSkincareMerchandise}=await import("data:text/javascript;base64,"+Buffer.from(js).toString("base64"));
...
test("unpublished skincare is visible but cannot acquire inventory from editorial data",()=>{
  const result=mergeSkincareMerchandise([], [upcoming,{id:"supplement",category:"supplements"}]);
...
test("a live Shopify record owns price, currency, stock and variants without duplication",()=>{
  ...
  const result=mergeSkincareMerchandise([live],[upcoming]);
  assert.equal(result.length,1);
